...
/**
 * The Class HistogramPanel displays an interactive histogram from any linear
 * set of data.
 */
public class HistogramPanel extends JPanel implements Runnable, ChangeListener {
	...
	public HistogramPanel(float[] data) {
		...
		for (int i = 1; i < data.length; i++) {
			if (data[i] == Double.NaN || data[i] == Double.NEGATIVE_INFINITY || data[i] == Double.POSITIVE_INFINITY)
				continue;
			if (data[i] < minDataValue)
				minDataValue = data[i];
			else if (data[i] > maxDataValue)
				maxDataValue = data[i];
		}
	...
	private float[] removeNaN(float[] data) {
		...
		for (; lastValidIndex > 0; lastValidIndex--) {
			if (!Float.isNaN(data[lastValidIndex]))
				break;
		}
	...
	public void run() {
		...
		// from "+currentMinValue+" to "+currentMaxValue);
		...
		// System.out.println("Calculating "+currentCategoryCount+" categories
		// from "+currentMinValue+" to "+currentMaxValue+" with interval
		// "+interval);

		for (int c = 0; c < categories.length; c++) {
			categories[c] = new HistogramCategory(currentMinValue + (interval * c),
					currentMinValue + (interval * (c + 1)));
			// System.out.println("Made category from
			// "+categories[c].minValue+"-"+categories[c].maxValue);
		}
		...
		for (int d = 0; d < data.length; d++) {
			...
			if (data[d] == Double.NaN || data[d] == Double.NEGATIVE_INFINITY || data[d] == Double.POSITIVE_INFINITY)
				continue;

			// We need to put in an explicit check for the max value otherwise
			// we
			// get stupid boundary errors where our int rounds down when it
			// shouldn't
			...
			if (index < 0) {
				...
				// System.err.println("Histogram index "+index+" < 0 for value
				// "+data[d]+" minValue="+minDataValue+" interval="+interval);
				continue;
			}
			if (index >= categories.length) {
				// This just means we've zoomed past this point in the data
				// System.err.println("Histogram index "+index+" > categories
				// for value "+data[d]+" minValue="+minDataValue+"
				// interval="+interval);
				continue;
			}
	...
	private class MainHistogramPanel extends JPanel implements MouseListener, MouseMotionListener {
		...
		private int X_AXIS_SPACE = 50; // This will vary once we've calculated
										// the scale
		...
		public void setData(HistogramCategory[] categories) {
			...
			maxCount = 0;
			for (int c = 0; c < categories.length; c++) {
				if (categories[c].count > maxCount)
					maxCount = categories[c].count;
			}
		...
		public void paintComponent(Graphics g) {
			...
			AxisScale yAxisScale = new AxisScale(0, maxCount, this);
			...
			if (Double.isInfinite(currentMaxValue) || Double.isNaN(currentMaxValue)
					|| Double.isInfinite(currentMinValue) || Double.isNaN(currentMinValue)) {
			...
			if (isSelecting) {
				...
				g.fillRect(Math.min(selectionStart, selectionEnd), 5, Math.abs(selectionStart - selectionEnd),
						getHeight() - (Y_AXIS_SPACE + 5));
				g.setColor(Color.BLACK);
			}
			...
			while (currentYValue < maxCount) {
				...
				g.drawString(yText, (X_AXIS_SPACE - 6) - g.getFontMetrics().stringWidth(yText),
						(int) ((getHeight() - Y_AXIS_SPACE) - yHeight) + (g.getFontMetrics().getAscent() / 2));
				g.drawLine(X_AXIS_SPACE - 3, (int) ((getHeight() - Y_AXIS_SPACE) - yHeight), X_AXIS_SPACE,
			...
			if (categories.length > 0) {
				AxisScale xAxisScale = new AxisScale(currentMinValue, currentMaxValue, this);
				...
				while (currentXValue < currentMaxValue) {
					...
					g.drawString(xText, getX(currentXValue) - (g.getFontMetrics().stringWidth(xText) / 2),
							(int) ((getHeight() - Y_AXIS_SPACE) + 15));
					g.drawLine(getX(currentXValue), getHeight() - Y_AXIS_SPACE, getX(currentXValue),
							(getHeight() - Y_AXIS_SPACE) + 3);
			...
			for (int c = 0; c < categories.length; c++) {
				...
				g.fillRect(categories[c].xStart, (int) ((getHeight() - Y_AXIS_SPACE) - ySize),
						categories[c].xEnd - categories[c].xStart, (int) (ySize));

				// Draw a box around it
				g.setColor(Color.BLACK);
				g.drawRect(categories[c].xStart, (int) ((getHeight() - Y_AXIS_SPACE) - ySize),
						categories[c].xEnd - categories[c].xStart, (int) (ySize));
		...
		public int getX(double xValue) {
			return X_AXIS_SPACE + (int) (((getWidth() - (X_AXIS_SPACE + 5)) / (currentMaxValue - currentMinValue))
					* (xValue - currentMinValue));
		}

		public double getXValue(int xPosition) {
			return ((((double) (xPosition - X_AXIS_SPACE)) / (getWidth() - (X_AXIS_SPACE + 5)))
					* (currentMaxValue - currentMinValue)) + currentMinValue;
		}
		...
		public void mouseMoved(MouseEvent me) {
			...
			// If we're outside the main plot area we don't need to worry about
			// it
		...
		public void mouseReleased(MouseEvent me) {
			...
			if (isSelecting) {
				...
				if ((getXValue(Math.max(selectionStart, selectionEnd))
						- getXValue(Math.min(selectionStart, selectionEnd))) < 0.0001)
					return;
	...
	private class StatusPanel extends JPanel {
		...
		public void setSelectedCategory(HistogramCategory category) {
			if (category == null) {
				...
				label.setText("Value Range = " + df.format(category.minValue) + " to " + df.format(category.maxValue)
